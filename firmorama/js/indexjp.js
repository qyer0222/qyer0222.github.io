...
//				})
	...
	window.onscroll = function(){
			...
			//判断header是否跟随
			if(window.pageYOffset+56>=height){
				...
				if($('#con_head')[0].className){
					...
					$('#con_head ul').css('border-top','3px solid #0ee172');
				}
			}
			//点击后可以滚动到top0
			if(window.pageYOffset>2000){
				$('#back_Top').css('display','block').mouseover(function(){
					$('#back_Top').css('transform','rotate(0)')
				}).click(function(){
					window.scrollTo(0,0)
				});
			}else{
			...
			$('#back_Top').mouseout(function(){
					$('#back_Top').css('transform','rotate(-30deg)')
				})
...
/*	function mover(){
		$('#con_body img').mouseover(function(){
			$(this).find('div').css({
				display:'block',
				opacity: '1'
	...
	function getdata(){
		...
		//生成div
		function craetdiv(){
			var Width = Math.floor($('header').width()/53);//求可以生成多少div当前是19个
			var Height = 7;
			var str = ''
			for(var i=0;i<Width;i++){
				for(var j=0;j<Height;j++){
					str+='<div style="left:'+i*53+'px;top:'+j*53+'px;background-position:'+(-i*50)+'px '+(-j*50)+'px;"></div>';
				}
			}
			$('#banner')[0].innerHTML = str;
		}
		function Mouseover(){
